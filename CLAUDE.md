...
A Kotlin Multiplatform (KMP) project that looks up the user's public IP, city, country, timezone, and coordinates via [ipapi.co](https://ipapi.co) and renders the location as an OpenStreetMap static map URL. The same business logic powers apps on **Android, iOS, iPadOS, macOS, Linux, Windows, and the Web**.
...
- VPN handling is implicit: ipapi.co returns whichever IP the request egressed from, which is the VPN's exit IP when a VPN is active.
...
- **No hard-coded strings** — use constants (e.g., the ipapi endpoint and OSM static-map base URL are private consts in their callers).
